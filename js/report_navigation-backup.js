	...
	var processData = function(options, theData){
		...
		try {
			...
			$.each(breakdown, function(ind,data){
				...
				let needsBreak = activity.indexOf('[') === 0 ? true : false;
				let noBreak = false;
				let addNextLevel = false;

				if (needsBreak) {
	     
					let splitters = activity.split('] ');
					let firstSplit = splitters[0];
					let reSplit = firstSplit.split('[');
					activity = reSplit[1];
					var otherDetail = splitters[1];
					var otherDetailAltName = otherDetail.substring(0,12) + '..';
	   				// // console.log(activity + ' --> ' + otherDetail);
				...
				} else if (activity === 'typed/bookmarked') {
					activity = 'Direct Load';
					noBreak = true;
				}/*
				else {
					var furtherBreak = data['breakdown'];
					var firstTrackCodeName = furtherBreak[0]['name'];
			        if (firstTrackCodeName !== "::unspecified::") {
			        	activity = activity + " (" + firstTrackCodeName + ")";
			        }
					noBreak = true;
				}
				*/
					...
					var firstTrackCodeName = furtherBreak[0]['name'];
			        if (firstTrackCodeName !== "::unspecified::") {
			        	var otherDetailAltName = activity + " (" + firstTrackCodeName + ")";
			        	var otherDetail = activity + " (" + firstTrackCodeName + ")";
			        } else {
			        	var otherDetail = activity;
			        	var otherDetailAltName = activity;
			        }
				...
				if (typeof finalData['Navigation'][activity] === "undefined") {
		        	...
		        	$.each(metrics, function(index,met){
		          		finalData['Navigation'][activity][met['name']] = 0;
		        	});
		      	} 
				...
				var totalsData = data['counts'];
			      ...
			      if (needsBreak) {
			      	fD['breakdown'][otherDetail] = {
			      		'altName' : otherDetailAltName
			      	};
			        var bN = fD['breakdown'][otherDetail];
			        $.each(metrics, function(index,met){
						bN[met['name']] = 0;
					});
			        $.each(totalsData, function(x,num){
						...
						bN[which] = parseInt(num);
			          	fD[which] = parseInt(fD[which]) + parseInt(num);
			        });
			      }
			      else {
			      	$.each(totalsData, function(x,num){
